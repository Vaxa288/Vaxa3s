...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.add_img = tk.PhotoImage(file='roflan.gif')
        btn_open_dialog = tk.Button(toolbar, text='Додати', command=self.open_dialog, bg='#d7d8e0', bd=0,
                                    compound=tk.TOP, image=self.add_img)
        btn_open_dialog.pack(side=tk.LEFT)

        self.update_img = tk.PhotoImage(file='roflanHD.gif')
        btn_edit_dialog = tk.Button(toolbar, text='Редагувати', bg='#d7d8e0', bd=0, image=self.update_img,
                                    compound=tk.TOP, command=self.open_update_dialog)
        btn_edit_dialog.pack(side=tk.LEFT)

        self.delete_img = tk.PhotoImage(file='gp.gif')
        btn_delete = tk.Button(toolbar, text='Видалити', bg='#d7d8e0', bd=0, image=self.delete_img,
                               compound=tk.TOP, command=self.delete_records)
        btn_delete.pack(side=tk.LEFT)

        self.student_img = tk.PhotoImage(file='student.gif')
        btn_delete = tk.Button(toolbar, text='Студент', bg='#d7d8e0', bd=0, image=self.student_img,
                               compound=tk.TOP, command=self.delete_records)
        btn_delete.pack(side=tk.RIGHT)
        self.student1 = tk.PhotoImage(file='student.gif')
        btn_delete = tk.Button(toolbar, text='Викладачі', bg='#d7d8e0', bd=0, 
                               compound=tk.TOP, command=self.delete_records)
        btn_delete.pack(side=tk.RIGHT)
        self.student2 = tk.PhotoImage(file='student.gif')
        btn_delete = tk.Button(toolbar, text='Предмети', bg='#d7d8e0', bd=0, 
                               compound=tk.TOP, command=self.delete_records)
        btn_delete.pack(side=tk.RIGHT)
        ...
        self.tree.heading('description', text='Викладачі')
        self.tree.heading('costs', text='Курси')
        self.tree.heading('total', text='Кількість годин')
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        label_description = tk.Label(self, text='Викладачі:')
        label_description.place(x=50, y=50)
        label_select = tk.Label(self, text='Курси:')
        label_select.place(x=50, y=80)
        label_sum = tk.Label(self, text='Кількість годин:')
        ...
        self.combobox = ttk.Combobox(self, values=[u'Матаналіз', u'ООП', u'Вища математика', u'Схемотехніка', u'Дискретна математика' ])
